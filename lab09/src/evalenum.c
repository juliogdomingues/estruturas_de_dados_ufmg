...
// Saida: str
 ...
 sprintf(str,"%s cmp %ld move %d calls %d", pref, s->cmp, s->move, s->calls); 
 return str;
}
...
// heapsort
void heapify(int * A, int Esq, int Dir, sortperf_t *s) {
    inccalls(s, 1); // Adiciona contagem de chamada de função para heapify
    int i = Esq;
    int j = 2 * i; // Filho da esquerda
