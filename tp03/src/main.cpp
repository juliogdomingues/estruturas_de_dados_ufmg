...
int main() {
    ...
    for (int i = 0; i < nOperacoes; i++) {
        ...
        if (operacao == 'u') {
            ...
            Matrix newMatrix(a, b, c, d);
            st.update(tempo, newMatrix, 1, 0, MAX - 1);
        } else if (operacao == 'q') {
            int t0, td, x, y;
            std::cin >> t0 >> td >> x >> y;

            Matrix result = st.query(t0, td, 1, 0, MAX - 1);
            int finalX = (result.m[0][0] * x + result.m[0][1] * y) % MOD;
            int finalY = (result.m[1][0] * x + result.m[1][1] * y) % MOD;
